...
- [ ] Shoe-finder product tagging — staging's tag vocabulary (`dry-course`, `soft-course`, `spiked`, `flat`, `traditional-leather`, `modern-style`) is in the JSON but not yet applied to US products. Client owns adding these tags to products in `/collections/mens-golf-shoes` + `/collections/womens-golf-shoes` so the filter resolves beyond Q1 (gender).
- [x] ~~Shoe-finder Q4 colour images (Black / White / Other tiles)~~ Resolved: no longer relevant — the colour question was removed when the shoe-finder was re-ported to staging parity (staging has no colour question).
